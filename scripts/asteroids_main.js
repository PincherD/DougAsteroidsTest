...
function onloadHandler()
{
   ...
   g_backgroundImg.src = 'images/gallery.png';
   ...
   extend(Asteroids.AttractorScene, Game.Scene,
   {
      ...
      onRenderScene: function onRenderScene(ctx)
      {
         if (this.imagesLoaded)
         {
            ...
            this.sineText(ctx, Game.Util.message("title"), GameHandler.width*0.5 - 180, GameHandler.height*0.5 - 64);
         }
         else
         {
            var t = (BITMAPS ? Game.centerFillText : Game.centerDrawText);
            t(ctx, Game.Util.message("please-wait"), "18pt Arial", GameHandler.height*0.5, "#ff0000");
         }
      },
      
      sceneRendererWelcome: function sceneRendererWelcome(ctx)
      {
         ctx.fillStyle = ctx.strokeStyle = "#e32626";
         var t = (BITMAPS ? Game.centerFillText : Game.centerDrawText);
      ...
      {
         ctx.fillStyle = ctx.strokeStyle = "rgb(226, 33, 33)";
         ...
         if (!iOS)
         {
            ...
            t(ctx, Game.Util.message("instruction7"), "14pt Arial", 40, 450);
         }
         else
         {
            t(ctx, Game.Util.message("instruction1"), "14pt Arial", 40, 320);
      ...
      sineText: function sineText(ctx, txt, xpos, ypos)
      {
         ...
         for (var i=0, j=txt.length; i<j; i++)
         {
            ...
            f(ctx, txt[i], "36pt Courier New", x + i*30, y, "#00c2ff");
         }
         this.sine += 0.075;
      },
   ...
   extend(Asteroids.GameScene, Game.Scene,
   {
      ...
      generateBabyAsteroids: function generateBabyAsteroids(asteroid, parentVector)
      {
         ...
         if (asteroid.size > 1)
         {
            ...
            {
               var babySize = randomInt(3, asteroid.size - 3);
